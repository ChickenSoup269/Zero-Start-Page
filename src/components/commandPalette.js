...
export function initCommandPalette(options = {}) {
  ...
  const eventToShortcut = (e) => {
    ...
    if (!key || ["Alt", "Control", "Ctrl", "Shift", "Meta"].includes(key))
      return ""
    const parts = []
  ...
  const normalizeShortcut = (value = "") =>
    String(value).toLowerCase().replace(/\s+/g, "").replace("control+", "ctrl+")
  ...
  const commands = [
    ...
    },
    {
      ...
      shortcut: "",
      keywords:
        "ambient sound white noise mua mua rao bien song bien song cafe campfire focus thu gian",
      action: async () => {
    ...
    },
    {
      ...
      action: () =>
        toggleCheckbox(showBookmarkGroupsCheckbox, "widget_bookmark_groups"),
    },
    {
      id: "toggle-gregorian",
    ...
    },
    {
      ...
      action: () =>
        toggleCheckbox(showTopRightControlsCheckbox, "widget_google_apps"),
    },
    {
  ...
  function setBgSize(size) {
    ...
    if (bgSizeSelect) {
      ...
      if (size === "cover") labelText = i18n.bg_cover || "Fill (Cover)"
        ...
        labelText = i18n.bg_contain || "Fit (Contain)"
      ...
      if (showUndo) {
        ...
        showToast(`${stateText}: ${labelText}`, {
          undoFn: () => {
            ...
            clockDisplaySelect.dispatchEvent(
  ...
  const loadHideAllState = () => {
    try {
      return (
        JSON.parse(localStorage.getItem(HIDE_ALL_STATE_KEY) || "null") || {}
      )
    } catch (e) {
      return {}
    }
  ...
  function toggleHideAll() {
    if (!allHidden) {
      ...
      const clockDisplaySelect = document.getElementById("clock-display-select")
      if (clockDisplaySelect && clockDisplaySelect.value !== "hide")
        toggleClock()
  ...
  function renderList() {
    ...
    listContainer.querySelectorAll(".cp-shortcut-edit").forEach((button) => {
      button.addEventListener("click", (event) => {
        ...
        showToast(
          i18n.cmd_listening_toast ||
            "Press new shortcut combination. Esc to cancel, Backspace to unassign.",
        )
      })
    })
  ...
  searchInput.addEventListener("input", (e) => {
    ...
    filteredCommands = commands.filter((c) =>
      normalizeText(
        `${c.title} ${c.desc} ${c.shortcut || c.shortcutHint || ""} ${c.keywords || ""}`,
      ).includes(query),
    )
    selectedIndex = 0
    renderList()
    ...
    (e) => {
      ...
      if (shortcutCaptureCommand) {
        ...
        if (shortcut && normalizeShortcut(shortcut) === "ctrl+k") {
          showToast(
            i18n.cmd_ctrl_k_reserved ||
              "Ctrl + K is reserved to open the Command Palette",
          )
          return
        }
        const duplicate =
          ...
          commands.find(
            (cmd) =>
              cmd.id !== shortcutCaptureCommand.id &&
              normalizeShortcut(cmd.shortcut) === normalizeShortcut(shortcut),
          )
        if (duplicate) {
          const dupMsg = (
            i18n.cmd_duplicate_error || "This key is already used for: {title}"
          ).replace("{title}", duplicate.title)
          showToast(dupMsg)
          return
        }

        const fallbackShortcut =
          FALLBACK_SHORTCUTS[shortcutCaptureCommand.id] || ""
        ...
        const label =
          shortcutCaptureCommand.shortcut || i18n.cmd_unassigned || "unassigned"
        const updatedMsg = (
          i18n.cmd_updated_toast || "Updated shortcut: {title} ({shortcut})"
        )
        ...
        filteredCommands = commands.filter((c) =>
          normalizeText(
            `${c.title} ${c.desc} ${c.shortcut || c.shortcutHint || ""} ${c.keywords || ""}`,
          ).includes(normalizeText(searchInput.value)),
        )
        renderList()
        return
      }
