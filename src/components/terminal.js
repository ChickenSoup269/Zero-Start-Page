export function initTerminal() {
  ...
  function applyTheme() {
    if (currentTheme === "powershell") {
      ...
      document
        ...
        .forEach((btn) => (btn.style.color = "#000"))
      ...
      body.style.fontFamily =
        "'Ubuntu Mono', 'JetBrains Mono', Consolas, monospace"
  ...
  document
    .querySelectorAll(".mac-close, .win-close")
    .forEach((btn) => btn.addEventListener("click", closeTerminal))
  document
    .querySelectorAll(".mac-min, .win-min")
    .forEach((btn) => btn.addEventListener("click", closeTerminal))
  ...
  const toggleMaximize = () => {
    if (!isMaximized) {
      ...
      terminalWindow.style.borderRadius =
        terminalWindow.dataset.prevRadius || radius
      isMaximized = false
    }
    setTimeout(
      () =>
        (terminalWindow.style.transition =
          "opacity 0.2s cubic-bezier(0.16, 1, 0.3, 1), transform 0.2s cubic-bezier(0.16, 1, 0.3, 1), background 0.3s, border-radius 0.3s"),
      300,
    )
  }

  document
    .querySelectorAll(".mac-max, .win-max")
    .forEach((btn) => btn.addEventListener("click", toggleMaximize))
  ...
  function openTerminal() {
    ...
    requestAnimationFrame(() => {
      ...
      terminalWindow.style.transform = terminalWindow.dataset.dragged
        ? "none"
  ...
  input.addEventListener("keydown", async (e) => {
    if (e.key === "Enter") {
      ...
      let p = ""
      if (currentTheme === "powershell")
        p = `<span style="color: #cccccc;">PS C:\\Users\\${currentUser}&gt;</span>`
      else if (currentTheme === "linux")
        p = `<span style="color: #8ae234;">${currentUser}@ubuntu</span>:<span style="color: #729fcf;">~</span>$`
      else
        p =
          '<span style="color: #98c379;">➜</span> <span style="color: #61afef;">~</span>'
      ...
      if (cmd === "start") {
        ...
        try {
          ...
          parsedSettings = { error: "Failed to parse" }
        }
          ...
          const checks = [
            ...
            },
            {
              ...
              test: () => {
                ...
                return (
                  clock &&
                  clock.innerText.length > 0 &&
                  !clock.innerText.includes("NaN")
                )
              },
            },
            {
  ...
  <span style="color: #61afef;">ai</span>       - Chat with Gemini AI (requires API key)`)
        ...
        if (prompt === "setkey") {
          ...
          try {
            ...
            const answerSpan = document.getElementById(resId)
            if (answerSpan)
              answerSpan.innerHTML = `<span style="color: #e06c75;">Failed to fetch: ${e.message}</span>`
          }
        }
      } else if (cmd === "echo") {
        printOut(`\n${val.substring(5)}`)
