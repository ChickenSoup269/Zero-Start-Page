...
import { getBookmarks, setBookmarks, saveBookmarks, getBookmarkGroups, setBookmarkGroups, getActiveGroupId, setActiveGroupId } from "../services/state.js"
...
export function showContextMenu(x, y, index, type = 'bookmark', id = null, callbacks = null) {
...
function handleEdit() {
  ...
  if (contextMenuTargetType === 'bookmark') {
      ...
      if (group) {
          const newName = prompt(i18n.prompt_rename_group || "Enter new group name:", group.name)
...
function handleDelete() {
  ...
  if (contextMenuTargetType === 'bookmark') {
      ...
      const group = groups.find(g => g.id === contextMenuTargetId)
      if (group) {
           if (confirm(`${i18n.alert_delete_group_confirm || "Delete group"} "${group.name}"?`)) {
                // Prevent deleting if it's the only group? (Optional, but UI might break if no groups)
                if (groups.length <= 1) {
                    alert(i18n.alert_cannot_delete_last_group || "Cannot delete the last group.")
                    hideContextMenu()
                    return
                }
