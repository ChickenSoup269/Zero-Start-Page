export class PixelCubes {
  ...
  updateColor(color) { this.color = color; }
  ...
  draw() {
    ...
    for (let c of this.cubes) {
        ...
        if (z > 50) {
            ...
            let p = v.map(pt => {
                let x1=pt[0]*s, y1=(pt[1]*crx-pt[2]*srx)*s, z1=(pt[1]*srx+pt[2]*crx)*s;
                let x2=x1*cry+z1*sry, y2=y1;
                let x3=x2*crz-y2*srz, y3=x2*srz+y2*crz;
