...
export class NorthernLightsEffect {
    ...
    this.curtains.forEach((curtain) => {
      ...
      for (let i = 0; i < curtain.segments; i++) {
        ...
        const opacity =
          ...
          brightness
        const hue =
          (curtain.baseHue + Math.sin(i * 0.1 + curtain.phase) * 15) % 360
  ...
  _renderClassic(ctx, W, H) {
    ...
    this.particles.forEach((p, i) => {
      ...
      p.life -= 0.0025
      if (p.life <= 0 || p.y < -20)
        this.particles[i] = this._newParticleClassic(false)
