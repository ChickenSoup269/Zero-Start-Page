...
export class RainHDEffect {
  ...
  static LAYERS = [
    [8, 14, 10, 20, 0.06, 0.14, 0.35, 0.7], // 0 bg  — small, slow, faint
    [18, 29, 22, 38, 0.2, 0.38, 0.72, 1.3], // 1 mid — medium
    [32, 54, 40, 72, 0.45, 0.8, 1.0, 1.9], // 2 fg  — large, fast, bright
  ]
  ...
  setOptions(options = {}) {
    ...
    if (options.targetFps !== undefined) {
      this.fps = Math.max(24, Math.min(60, Number(options.targetFps) || 60))
      this.fpsInterval = 1000 / this.fps
    }
