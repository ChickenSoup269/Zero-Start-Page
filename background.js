...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    chrome.tabs.query({ audible: true }, (tabs) => {
      if (tabs.length === 0) {
        ...
        chrome.tabs.query({}, (allTabs) => {
          const tab = allTabs.find(t => t.url.includes("youtube.com") || t.url.includes("spotify.com") || t.url.includes("music.youtube.com"));
          if (tab) {
  ...
  if (request.action === "mediaControl") {
    chrome.tabs.query({ audible: true }, (tabs) => {
      ...
      chrome.tabs.query({}, (allTabs) => {
        const targetTab = tabs[0] || allTabs.find(t => t.url.includes("youtube.com") || t.url.includes("spotify.com") || t.url.includes("music.youtube.com"));
        if (targetTab) {
            ...
            func: (command) => {
              ...
              switch (command) {
                case 'playPause':
                  ...
                  break;
                case 'next':
                  (document.querySelector('.ytp-next-button') || document.querySelector('[data-testid="control-button-skip-forward"]'))?.click();
                  break;
                case 'prev':
                  (document.querySelector('.ytp-prev-button') || document.querySelector('[data-testid="control-button-skip-back"]'))?.click();
                  break;
              }
            },
...
function getMediaFromTab(tabId, sendResponse) {
  chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      const ytTitle = document.querySelector('h1.ytd-watch-metadata yt-formatted-string')?.textContent || 
                     document.querySelector('yt-formatted-string.ytd-video-primary-info-renderer')?.textContent;
      const ytArtist = document.querySelector('yt-formatted-string.ytd-channel-name')?.textContent || 
                      document.querySelector('a.ytp-title-expanded-channel-link')?.textContent;
      
      const spotifyTitle = document.querySelector('[data-testid="now-playing-widget"] [data-testid="context-item-link"]')?.textContent;
      const spotifyArtist = document.querySelector('[data-testid="now-playing-widget"] [data-testid="context-item-info-subtitles"]')?.textContent;

      return {
        title: metadata?.title || ytTitle || spotifyTitle || document.title.replace(/^\(\d+\)\s/, ""),
